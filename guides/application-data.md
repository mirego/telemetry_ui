...
`telemetry_ui` provides a way to use the existing metrics helpers with application-defined data:
...
The `data_resolver` option is specified here to return the data that will be displayed in the same graph layout as the `telemetry_ui_events` entries.
The built-in metrics use the same `data_resolver` option: `data_resolver: &{:async, fn -> TelemetryUI.metric_data(&1, metric, &2) end}`.
