defmodule TelemetryUI.Web.VegaLite do
  ...
  defp title(assigns) do
    ~H"""
    <%= if @title do %>
      <h2 id={@id <> "-title"} class="hidden flex items-baseline gap-2 text-base opacity-80 mb-2">
        <%= @title %>
      </h2>
    <% end %>
    """
  ...
  defp loading_view(assigns) do
    ~H"""
    <div id={@id <> "-loading"} class="absolute top-[43px] left-0 flex flex-col grow items-center justify-center py-3 w-full">
      <div class="p-3 px-5 gap-1 flex flex-col grow items-center">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" stroke="currentColor" class="animation-loading opacity-20" width="24px">
