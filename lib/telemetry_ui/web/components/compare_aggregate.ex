defmodule TelemetryUI.Web.Components.CompareAggregate do
  ...
  def fill_expression(metric) do
    ...
    "datum.compare_percentage === 0 ? 'grey' : datum.compare_percentage > 0 ? '#{good}' : '#{bad}'"
  end

  def compare_icon_expression(_options) do
    "datum.compare_percentage >= 0 ? '▲ ' : '▼ '"
  end
  ...
  def spec(options) do
    ...
    |> Vl.transform(
      calculate:
        "datum.compare_aggregate_value['aggregate_value'] > 0 ? (datum.current_aggregate_value['aggregate_value']-datum.compare_aggregate_value['aggregate_value'])/datum.compare_aggregate_value['aggregate_value']*100 : 0",
    ...
    |> Vl.transform(
      calculate:
        "datum.compare_percentage !== 0 ? datum.compare_percentage_icon+(round(abs(datum.compare_percentage) * 100) % 100 === 0 ? format(abs(datum.compare_percentage), '.0f') : format(abs(datum.compare_percentage), '.2f'))+'%' : '-'",
